...
export const useVaultStore = defineStore("vaultStore", () => {
  ...
  const createVault = (vault?: IVaultFile): IVaultFile => {
    return {
      id: vault?.id || cryptoStore.randomUUID(),
  ...
  const addVault = (newVault: IVaultFile): boolean => {
    ...
    try {
      ...
      vaults.value.push(createVault(newVault))
    }
    return true

  }

  const updateVault = (newVault: IVaultFile): boolean => {
    const index = vaults.value.findIndex(vault => vault.id === newVault.id)

    if (index < 0)
      return false

    const updateVault = createVault(newVault)
  ...
  // KEY SECTION BEGIN
  const createKey = (key?: IVaultKey, options?: ICreateKeyOptions): IVaultKey => {
    return {
      id: options?.forceNewId ? cryptoStore.getUUID() : key?.id || cryptoStore.getUUID(),
